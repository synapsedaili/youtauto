...
def _create_text_image(text: str, width: int, height: int, fontsize: int, is_shorts: bool) -> Image.Image:
    """Pillow ile metin içeren görsel oluşturur (ImageMagick yoksa kullanılır)."""
    img = Image.new("RGB", (width, height), (0, 0, 0))
    draw = ImageDraw.Draw(img)
    
    # Font yükleme (güvenli)
    try:
...
def _create_text_clips(script: str, width: int, height: int, is_shorts: bool, duration: float) -> list:
    ...
    for line in lines:
        ...
        try:
            ...
            txt_clip = txt_clip.set_position(("center", "bottom"))  # 👈 YAZI ALT KISIMDA
...
def create_shorts_video(audio_path: str, script: str, output_path: str):
    ...
    final_video = CompositeVideoClip([background] + text_clips)  # ✅ Arka plan önce, yazılar sonra
