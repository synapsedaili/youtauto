...
def create_video_with_chunks(script, output_path, is_shorts=True):
    ...
    # AI metninin tahmini süresini hesapla
    word_count = len(script.split())
    avg_word_duration = 0.3  # Saniye başına ortalama kelime (human speech rate)
    estimated_duration = word_count * avg_word_duration
    
    # Maksimum süreye göre sınırla
    if is_shorts:
        total_duration = min(estimated_duration, Config.MAX_SHORTS_DURATION)
    else:
        total_duration = min(estimated_duration, Config.MAX_PODCAST_DURATION)
    
    logger.info(f"📊 Tahmini metin süresi: {estimated_duration:.1f}sn, Kullanılacak: {total_duration:.1f}sn")
    ...
    with tempfile.TemporaryDirectory(dir=str(Config.TEMP_DIR)) as temp_dir:
        ...
        # Gerçek ses süresi ile video süresini eşitle
        actual_audio_duration = audio.duration
        total_duration = min(actual_audio_duration, total_duration)
