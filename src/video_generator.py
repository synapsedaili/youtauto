...
from src.tts import generate_voice_with_edge_tts  # Seslendirme fonksiyonu
...
def create_shorts_bg(image_list, total_duration, width, height):
    ...
    if not clips:
        logger.warning("⚠️ Hiç görsel bulunamadı → Siyah arka plan")
...
def create_fast_active_bg(bg_path, total_duration, width, height):
    ...
    if bg_path is None or not Path(str(bg_path)).exists():
        logger.warning(f"⚠️ Podcast görseli bulunamadı → Siyah arka plan: {bg_path}")
        return ColorClip((width, height), (0, 0, 0)).set_duration(total_duration)

    logger.info(f"🖼️ Podcast görseli kullanılıyor: {bg_path}")
    ...
    active_part = img.set_duration(zoom_dur).resize(lambda t: 1 + 0.02 * (t / zoom_dur))  # Daha yavaş zoom
    if static_dur > 0:
        static_part = img.set_duration(static_dur).resize(1.02)  # Minimal zoom
...
def create_video_with_chunks(script, output_path, is_shorts=True):
    ...
    with tempfile.TemporaryDirectory(dir=str(Config.TEMP_DIR)) as temp_dir:
        ...
        # SESLİNDİRME BURADA ÇAĞRILIYOR - asyncio import hatası buradan geliyordu
        # ama artık yukarıda import ettik, bu yüzden hata olmaz
        ...
        # Metni parçalara böl
        words = script.split()
        chunk_size = 4 if is_shorts else 185
...
def create_shorts_video(audio_path: str, script: str, output_path: str):
    ...
    # Burada seslendirme artık yapılmaz, çünkü run_pipeline.py içinde zaten yapılmıştır
    # Bu yüzden sadece video üretir
...
def create_podcast_video(audio_path: str, script: str, output_path: str):
    """Geriye uyumluluk için - yeni fonksiyona yönlendirir."""
    # Burada da sadece video üretimi yap
