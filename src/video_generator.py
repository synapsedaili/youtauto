...
def create_video_with_chunks(script, output_path, is_shorts=True):
    ...
    with tempfile.TemporaryDirectory(dir=str(Config.TEMP_DIR)) as temp_dir:
        ...
        # 👇 YENİ SİSTEM: SES SÜRESİNE GÖRE METİN PARÇALAMA
        words = script.split()
        text_clips = []
        start_time = 0.0
        
        # Her kelime için tahmini süre (saniye)
        avg_word_duration = total_duration / len(words) if words else 0.3
        
        # Satır başına kelime sayısı
        words_per_line = 4 if is_shorts else 8  # Shorts: 4 kelime, Podcast: 8 kelime
        ...
        for i in range(0, len(words), words_per_line):
            ...
            # Bu satırın süresini hesapla
            line_duration = len(line_words) * avg_word_duration
            line_duration = max(1.0, min(line_duration, total_duration - start_time))  # Min 1sn
