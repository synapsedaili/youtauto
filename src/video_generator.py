...
from moviepy.editor import (
    ColorClip, CompositeVideoClip, AudioFileClip, ImageClip, concatenate_videoclips, TextClip
)
...
def _is_nvidia_gpu() -> bool:
    """Sistemde NVIDIA GPU var mı kontrol et."""
    try:
        import subprocess
...
def _create_text_image(text: str, width: int, height: int, fontsize: int, is_shorts: bool) -> Image.Image:
    """Pillow ile metin görseli oluştur (RAM'de)."""
    ...
    for line in lines[:3]:
        draw.text((50, y_offset), line, font=font, fill="white", stroke_width=2, stroke_fill="black")
        y_offset += fontsize + 10

    return img

def _create_text_clips(script: str, width: int, height: int, is_shorts: bool, duration: float) -> list:
    """Yazı kliplerini doğrudan RAM üzerinden oluşturur."""
    lines = script.split(". ")
    text_clips = []
    start_time = 0.0
    fontsize = 150 if is_shorts else 400
    ...
    for line in lines:
        ...
        seg_duration = max(1.3 if is_shorts else 2.4, min(word_count * (0.593 if is_shorts else 0.71), duration - start_time))
        ...
        try:
            txt_clip = TextClip(
                ...
                stroke_color="black",
                stroke_width=2
            ).set_position(("center", height - 400 if is_shorts else height - 300))
        except:
            img = _create_text_image(line, width, height, fontsize, is_shorts)
            txt_clip = ImageClip(np.array(img), duration=seg_duration)
            txt_clip = txt_clip.set_position(("center", height - 400 if is_shorts else height - 300))
...
def _get_script_chunk(script: str, start_time: float, end_time: float, is_podcast: bool) -> str:
    """Podcast script'ini zaman aralığına göre böl."""
    words = script.split()
...
class KenBurnsImageClip(ImageClip):
    """Görsel üzerinde çok hafif yakınlaşma/uzaklaşma efekti uygular."""
...
def create_shorts_video(audio_path: str, script: str, output_path: str):
    ...
    if image_path.exists():
        logger.info(f"🖼️ Görsel kullanılıyor: {image_path} (Ken Burns efektiyle)")
        background = KenBurnsImageClip(str(image_path), duration=total_duration, target_size=(width, height), zoom_factor=1.05)
    else:
        logger.warning(f"⚠️ Görsel bulunamadı → Siyah arka plan")
...
def create_podcast_video(audio_path: str, script: str, output_path: str):
    ...
    audio = AudioFileClip(audio_path)
    total_duration = min(audio.duration, Config.PODCAST_DURATION)  # 900 sn
    ...
    if image_path.exists():
        ...
        background_base = KenBurnsImageClip(str(image_path), duration=total_duration, target_size=(width, height), zoom_factor=1.03)
    else:
