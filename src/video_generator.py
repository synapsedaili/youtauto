...
import asyncio  # Bu vardı ama import eksikti
...
def create_video_with_chunks(script, output_path, is_shorts=True):
    ...
    with tempfile.TemporaryDirectory(dir=str(Config.TEMP_DIR)) as temp_dir:
        ...
        # Metni parçalara böl
        words = script.split()
        chunk_size = 4 if is_shorts else 250
        ...
        # Videoyu yaz (dosya boyutunu azaltmak için ayarlar)
        codec = "h264_nvenc" if _is_nvidia_gpu() else "libx264"
        preset = "fast" if _is_nvidia_gpu() else "medium"  # medium daha küçük dosya boyutu
        crf = 23  # 18-28 arası, 23 dengeli kalite-boyut
        ...
        final_video.write_videofile(
            ...
            threads=CORES,
            preset=preset,
            crf=crf,  # Dosya boyutunu azaltır
            logger=None
        )
        
        logger.info(f"✅ Video hazır: {output_path}")
        logger.info(f"📊 Video boyutu: {output_path.stat().st_size / (1024*1024):.2f} MB")
