...
def _create_text_clips(script: str, width: int, height: int, is_shorts: bool, duration: float) -> list:
    ...
    for line in lines:
        ...
        # 👇 KELİME DEĞİL KARAKTERE GÖRE SÜRE HESAPLA
        char_count = len(line)
        seg_duration = max(3.0, char_count * 0.1)  # Her karakter için 0.1 sn + minimum 3 sn
        
        # Toplam süreyi aşmamaya dikkat et
        if start_time + seg_duration > duration:
            seg_duration = duration - start_time
        ...
        try:
            ...
            txt_clip = ImageClip(np.array(img), duration=seg_duration, transparent=True)
