...
def create_podcast_bg_with_timed_effects(image_paths, total_duration, width, height):
    ...
    while elapsed < total_duration:
        ...
        if Path(img_path).exists():
            logger.info(f"🖼️ Podcast görseli: {img_path} ({elapsed:.1f}s - {elapsed + clip_duration:.1f}s)")
            ...
            if elapsed < 300:  # 0-5 dk: Sürekli Ken Burns
                ...
                logger.debug(f"  → Sürekli Ken Burns ({elapsed:.1f}s)")
                
            elif elapsed < 540:  # 5-9 dk: 30sn efekt VAR, 30sn efekt YOK
                # 30 saniyelik döngü içinde miyiz?
                ...
                if cycle_position < 30:  # İlk 30sn: Ken Burns VAR
                    ...
                    logger.debug(f"  → 30sn Ken Burns ON ({elapsed:.1f}s)")
                else:  # Son 30sn: Ken Burns YOK (statik)
                    clip = clip.set_duration(clip_duration).set_position('center')
                    logger.debug(f"  → 30sn Ken Burns OFF ({elapsed:.1f}s)")
                    
            else:  # 9+ dk: Efektsiz (statik)
                clip = clip.set_duration(clip_duration).set_position('center')
                logger.debug(f"  → Statik görsel ({elapsed:.1f}s)")
...
def create_shorts_bg_with_live_effect(image_paths, total_duration, width, height):
    ...
    while elapsed < total_duration:
        # Son görseldeyiz ve video bitmek üzere → başa DÖNME
        if image_index >= len(image_paths):
            # Son görseli video sonuna kadar uzat
            remaining_time = total_duration - elapsed
            if remaining_time > 0:
                last_img_path = image_paths[-1]
                logger.info(f"🖼️ Shorts son görsel uzatılıyor: {last_img_path} ({remaining_time:.1f}s)")
        ...
        if Path(img_path).exists():
            logger.info(f"🖼️ Shorts görseli #{image_index + 1}: {img_path} ({elapsed:.1f}s - {elapsed + clip_duration:.1f}s)")
...
def create_video_with_chunks(script, output_path, is_shorts=True):
    ...
    """
    logger.info(f"🎥 {'Shorts' if is_shorts else 'Podcast'} videosu üretiliyor (Özel Ken Burns zamanlaması)...")
    ...
    with tempfile.TemporaryDirectory(dir=str(Config.TEMP_DIR)) as temp_dir:
        ...
        if is_shorts:
            ...
            for i in range(1, 10):  # 7-9 görsel
            ...
            if not image_paths:
                for idx in [current_index - 1, current_index + 1, 1]:
            ...
            for i in range(1, 31):  # 20-30 görsel
                img_path = Config.DATA_DIR / "images" / "pod" / f"{current_index}_{i}.png"
                if img_path.exists():
                    image_paths.append(str(img_path))
            ...
            if not image_paths:
                for idx in [current_index - 1, current_index + 1, 1]:
                    img_path = Config.DATA_DIR / "images" / "pod" / f"{idx}.png"
                    if img_path.exists():
                        image_paths = [str(img_path)]
                        break
