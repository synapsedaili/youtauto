...
class OpenSourceAIAPI:
    ...
    def generate_with_lmstudio(self, topic: str, mode: str = "shorts") -> str:
        ...
        try:
            ...
            # LM Studio varsayılan API key'i yok
            headers = {
                "Content-Type": "application/json"
            }
            
            payload = {
                "model": "local-model",  # LM Studio'da yüklediğiniz model
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a professional YouTube content creator. All information must be accurate, well-researched, and factually correct. Never invent facts or make unverified claims. If you're unsure about something, say you don't know rather than guessing."
                ...
                "temperature": 0.7,
                "max_tokens": 1000 if mode == "shorts" else 5000,  # Artırıldı
                "stream": False
            }
    ...
    def generate_with_textgen(self, topic: str, mode: str = "shorts") -> str:
        ...
        try:
            ...
            payload = {
                ...
                "max_new_tokens": 1000 if mode == "shorts" else 5000,  # Artırıldı
                "temperature": 0.7,
...
"""
    ...
    def _validate_content(self, content: str, topic: str, mode: str) -> bool:
        ...
        # Tutarlılık ve doğruluk kontrolleri
        unreliable_phrases = [
        ...
        # Doğrulanabilirlik kontrolü - temel kurallar
        must_have_phrases = [
        ...
        if not has_verified_reference:
            ...
            # Sadece uyarı ver, tamamen reddetme (bazı konular için zor olabilir)
        
        # Konuyla ilgisizlik kontrolü
        ...
        # Call-to-action kontrolü (shorts için)
        if mode == "shorts":
...
"""
    ...
    def generate_script_content(self, topic: str, mode: str = "shorts") -> str:
        ...
        # Tüm AI'lar başarısız olursa kaliteli fallback
...
def generate_script(topic: str, mode: str = "shorts") -> str:
    """
    Dinamik script üretici - 3 açık kaynaklı AI ile fallback mekanizması
    ...
    Returns:
        str: Oluşturulan script
    """
    ai_api = OpenSourceAIAPI()
    
    # Script'i üret
    script = ai_api.generate_script_content(topic, mode)
    
    # Karakter limit kontrolü
    if mode == "shorts":
        limit = Config.SHORTS_CHAR_LIMIT
        if len(script) > limit:
            script = script[:limit]
            logger.warning(f"⚠️ Shorts script exceeded character limit ({len(script)}/{limit}), shortened")
    else:
        limit = Config.PODCAST_CHAR_LIMIT  # 45.000 karakter
        if len(script) > limit:
            script = script[:limit]
            logger.warning(f"⚠️ Podcast script exceeded character limit ({len(script)}/{limit}), shortened")
    
    logger.info(f"✅ {mode.upper()} script ready! ({len(script)} characters)")
    return script
...
if __name__ == "__main__":
    ...
    for i, topic in enumerate(selected_topics, 1):
        ...
        print("-" * 40)
        shorts_script = generate_script(topic, "shorts")
        print(shorts_script)
        ...
        print("-" * 40)
        podcast_script = generate_script(topic, "podcast")
