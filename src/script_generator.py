...
- Example: "But how did they hide it?" (NOT: "For the full story, listen to podcast!")
- NO podcast invitations, NO CTA, NO "like, comment, subscribe"
- Write micro-storytelling style, very concise
- Keep it under 830 characters total
...
Requirements:
- Write EXACTLY 500 words for this chapter
...
- Count your words: exactly 500 words required
...
Chapter {chapter_num} Content:"""
    ...
    def _generate_podcast_in_13_chapters(self, topic: str, generate_func: Callable) -> str:
        """Podcast metnini 13 chapter'da üretir (her chapter 500 words)."""
        full_script = ""
        ...
        for chapter_num in range(1, 14):
            ...
            try:
                ...
                logger.info(f"Chapter {chapter_num} produced {word_count} words (target: 500)")
                
                if word_count < 450:
                    # 500 altıysa genişlet
                    logger.info(f"🔄 Extending Chapter {chapter_num} ({word_count}/600)")
...
Extended Chapter {chapter_num} Content:"""
        ...
        if len(full_script) > 90000:  # 90K karakter (13 × 700 × 13 ≈ 9100 words)
            logger.info("Maximum character limit reached, truncating...")
            full_script = full_script[:90000]
