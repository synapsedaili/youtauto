...
CRITICAL RULES:
- TOTAL CHARACTERS: 1000-1200 (NOT 900!)
...
BEGINNING:"""
    ...
    def _generate_podcast_in_chunks(self, topic: str, generate_func: Callable) -> str:
        """
        Podcast'i 13 bölüme ayırarak 4 istekte üretir. TOPLAM ~36.000 KARAKTER HEDEFLENİR.
        Minimum 20.000 karakter garantisi ile.
        """
        # 📊 KARAKTER SAYILARINI 4X ARTIR
        chapter_ranges = [
            ("1-3", 5000),   # Eski: 2000 → Yeni: 8000
            ("4-6", 5000),   # Eski: 2000 → Yeni: 8000  
            ("7-9", 5000),   # Eski: 2000 → Yeni: 8000
            ("10-13", 8000) # Eski: 3000 → Yeni: 12000
        ]
        ...
        while retry_count <= MAX_RETRIES:
            ...
            for i, (chapters, max_chars) in enumerate(chapter_ranges):
                ...
                try:
                    chunk = generate_func(topic, "podcast", custom_prompt=prompt, timeout=1200)  # 20 dk timeout
                    if not chunk or len(chunk.strip()) < max_chars * 0.6:  # %60'dan azsa başarısız say
                        logger.warning(f"Chapter {chapters} çok kısa ({len(chunk)} karakter). Tekrar deneniyor...")
                        continue
                        
                    # Karakter sınırını uygula ama doğal noktalardan kes
                    if len(chunk) > max_chars:
                        chunk = self._smart_truncate(chunk, max_chars)
                    
                    full_script += chunk + "\n\n"
                    context = full_script[-800:]  # Bağlam için son 800 karakter
                    
                except Exception as e:
                    logger.warning(f"Chapter {chapters} üretim hatası: {e}. Tekrar deneniyor...")
                    continue
            
            # 🔍 KALİTE KONTROLÜ: Minimum 20.000 karakter
