...
def generate_script(topic: str, mode: str = "shorts") -> str:
   ...
   headers = {
      ...
      payload = {
          "inputs": prompt,
          "parameters": {
              "max_new_tokens": 300 if mode == "shorts" else 4000,
              "temperature": 0.75,
              "top_p": 0.9,
      ...
      # YENİ API URL (eski URL artık çalışmıyor!)
      response = requests.post(
          "https://api-inference.huggingface.co/models/meta-llama/Llama-3.2-1B",  # 👈 ESKİ
          # YENİ:
          "https://router.huggingface.co/models/meta-llama/Llama-3.2-1B",
          headers=headers,
          json=payload
      )
    ...
    if response.status_code != 200:
        ...
        # Hata durumunda örnek script döndür
        fallback_script = f"This is a test script for {topic}. Don't forget to like, comment, and subscribe!"
        return fallback_script[:max_chars]
