...
def clean_text(text: str) -> str:
    """Unicode temizliği + seslendirme için uygun hale getir."""
    import unicodedata
    # Seslendirme için gereksiz etiketleri temizle
    text = text.replace("[HOOK]", "").replace("[VOICE ON]", "").replace("[VOICE OFF]", "")
...
BEGINNING:"""
    ...
    def _generate_podcast_in_chunks(self, topic: str, generate_func) -> str:
        ...
        for i in range(3):
            ...
            try:
                # ⏱️ ZAMAN AŞIMI ARTIRILDI (120 → 300 saniye)
    ...
    def _generate_with_deepseek(self, topic: str, mode: str, custom_prompt=None, timeout=120) -> str:
        return self._generate_with_ollama_model(topic, mode, "deepseek-r1", custom_prompt, timeout)
    ...
    def _generate_enhanced_fallback(self, topic: str, mode: str) -> str:
        """Kaliteli fallback: konuya özel dinamik içerik"""
