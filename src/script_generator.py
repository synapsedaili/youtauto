...
Requirements:
- Generate 15 chapter titles for a podcast about this topic
...
Requirements:
- Write EXACTLY 300 words for this chapter
...
- Count your words: exactly 300 words required
...
Chapter {chapter_num} Content:"""
    ...
    def _generate_podcast_in_13_chapters(self, topic: str, generate_func: Callable) -> str:
        """Podcast metnini 13 chapter'da üretir (her chapter 300 words)."""
        full_script = ""
        ...
        for chapter_num in range(1, 6):
            logger.info(f"📖 Chapter {chapter_num}/5 generating...")
            ...
            try:
                ...
                logger.info(f"Chapter {chapter_num} produced {word_count} words (target: 250)")
                
                if word_count < 200:
                    # 200 altıysa genişlet
                    logger.info(f"🔄 Extending Chapter {chapter_num} ({word_count}/300)")
...
Extended Chapter {chapter_num} Content:"""
        ...
        if len(full_script) > 45000:  # 45K karakter (13 × 300 × 13 ≈ 3900 words)
