...
Bu modül, YouTube Shorts ve Podcast içerikleri için kaliteli metinler üretir.
Tüm içerikler tarihsel olarak doğrulanabilir, bilimsel temelli ve YouTube SEO kurallarına uygundur.

Özellikler:
- 3 katmanlı sağlayıcı sistemi (Qwen → Phi3 → Llama3.2)
- Podcast için 13 bölümlü yapı (4 istekte, 25.000+ karakter)
- Shorts'ta teknik talimatları engelleme
- Tarihsel doğruluk garantisi
...
CRITICAL RULES:
- TOTAL CHARACTERS: 950-1050 (NOT 1200!)
- Start with a NATURAL engaging historical statement (NOT "fact with" or "shocking fact")
- Include exactly 2 NATURAL curiosity questions (NOT "curious question #1")
- Example good start: "In 1965, Soviet scientists proposed a radical plan that would change everything..."
- Example good question: "How did they plan to wire an entire nation?" 
...
BEGINNING:"""
    ...
    def _generate_podcast_in_chunks(self, topic: str, generate_func: Callable) -> str:
        """
        Podcast'i 13 bölüme ayırarak 4 istekte üretir. TOPLAM ~25.000 KARAKTER HEDEFLENİR.
        Minimum 16.000 karakter garantisi ile.
        """
        # 📊 KARAKTER SAYILARINI İNDİR
        chapter_ranges = [
...
"""
    ...
    def _generate_with_qwen(self, topic: str, mode: str, custom_prompt=None, timeout=1200) -> str:
    return self._generate_with_ollama_model(topic, mode, "qwen2", custom_prompt, timeout)  
    
    def _generate_with_phi3(self, topic: str, mode: str, custom_prompt: Optional[str] = None, timeout: int = 1200) -> str:
        """Phi3 modeli ile metin üretir."""
        return self._generate_with_ollama_model(topic, mode, "phi3", custom_prompt, timeout)
    
    def _generate_with_ollama_fallback(self, topic: str, mode: str, custom_prompt: Optional[str] = None, timeout: int = 1200) -> str:
...
def generate_shorts_script(topic: str) -> str:
    ...
    # 👉 KRİTİK: 1050 KARAKTER SINIRI (67 saniyeye sığması için)
    if len(script) > 1050:
        script = script[:1050]
    ...
    # 👉 SON KONTROL: En az 800 karakter olsun
    if len(script) < 800:
...
def generate_podcast_script(topic: str) -> str:
    generator = AIScriptGenerator()
    script = generator.generate_script(topic, "podcast").strip()
    
    # Minimum karakter kontrolü (16.000 karakter)
    if len(script) < 16000:
        logger.warning(f"⚠️ Podcast script çok kısa ({len(script)} karakter). Kalite kontrolü tekrarlanacak.")
