...
# ======================
# ANA VİDEO ÜRETİMİ (SADECE DOSYA KAYITLI)
# ======================
# src/fast_generator.py (sadece video üretimi kısmı)

# src/fast_generator.py (video yazma kısmı)

def create_and_save_video(script, output_filename, is_shorts=True):
    ...
    # 👇 CRF YERİNE FFMPEG_PARAMS KULLAN
    final.write_videofile(
        ...
        preset="medium",        # 👈 Daha iyi sıkıştırma
        ffmpeg_params=['-crf', '23'],  # 👈 CRF burada
        logger=None
    )
    ...
    # Dosyayı output klasörüne taşı
    final_output_path = output_dir / output_filename
    shutil.move(temp_path, final_output_path)
    
    # Dosya boyutunu göster
    file_size_mb = os.path.getsize(final_output_path) / (1024 * 1024)
    print(f"✅ Video kaydedildi: {final_output_path} ({file_size_mb:.1f} MB)")
    
    # Geçici dosyaları temizle
    shutil.rmtree(temp_dir)
    
    print(f"🎉 {('Shorts' if is_shorts else 'Podcast')} tamamlandı!")
...
if __name__ == "__main__":
    ...
    print("🎉 Tüm işlemler tamamlandı! Videolar 'output/' klasöründe.")
