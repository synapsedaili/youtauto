...
async def generate_voice_with_edge_tts(text: str, output_path: str):
    ...
    try:
        ...
        rate = "+5%" if is_shorts else "+0%"
        pitch = "+10Hz" if is_shorts else "+0Hz"
        volume = "+2%" if is_shorts else "+0%"
        
        logger.info(f"⚡ Ses parametreleri: rate={rate}, pitch={pitch}, volume={volume}, voice={voice}")
        
        # 5. Edge TTS ile seslendirme
        ...
        async for chunk in communicate.stream():
            if chunk["type"] == "audio":
                logger.debug(f"🔊 Ses chunk {len(chunk['data'])} byte alındı")
            elif chunk["type"] == "word_boundary":
                logger.debug(f"🗣️ Kelime sınırı: {chunk['text']}")
        ...
        # Fallback olarak basit bir ses dosyası oluştur
        try:
            from gtts import gTTS
            logger.info("🔄 GTTS fallback moduna geçiliyor...")
            tts = gTTS("Error generating voice. Please try again later.", lang='en')
            tts.save(output_path)
        except Exception as fallback_e:
            logger.error(f"❌ GTTS fallback da başarısız: {str(fallback_e)}")
            raise
