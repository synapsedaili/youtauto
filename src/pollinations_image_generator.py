...
class PollinationsImageGenerator:
    ...
    def _create_safe_prompt(self, prompt: str, mode: str) -> str:
        ...
        # Add quality and safety parameters
        if mode == "podcast":
    ...
    def _generate_single_image(self, prompt: str, aspect_ratio: str, seq: int, mode: str) -> str:
        ...
        try:
            ...
            # Save image
            current_index = 1  # Get from utils if needed
    ...
    def _generate_podcast_images(self, topic: str, script: str) -> list:
        """Generate 25 podcast images."""
        images = []
        chapters = self._extract_chapters(script)
        
        # Generate from chapters
        for i, chapter in enumerate(chapters[:10]):  # First 5 chapters
        ...
        while len(images) < 26:
            prompt = f"Professional overview image of '{topic}' showing multiple related elements. Documentary style, museum exhibit quality."
    ...
    def _generate_shorts_images(self, topic: str, script: str) -> list:
        ...
        for i in range(11):
            if i < len(lines) and lines[i].strip():
                segment = lines[i][:150]
                prompt = f"Eye-catching, vertical composition image representing: '{segment}'. Social media optimized, mobile friendly."
            else:
                prompt = f"Engaging vertical image for '{topic}' - key moment or concept. Mobile optimized, social media friendly."
