...
def get_authenticated_service():
    ...
    # 2. GitHub Actions'ta: YOUTUBE_CREDENTIALS environment variable
    credentials_base64 = os.environ.get("YOUTUBE_CREDENTIALS")
    ...
    if not credentials_base64:
        logger.error("❌ YOUTUBE_CREDENTIALS environment variable bulunamadı!")
        raise RuntimeError("❌ YouTube kimlik doğrulaması başarısız!")
    ...
    # credentials.json decode et
    try:
        credentials_json = base64.b64decode(credentials_base64).decode("utf-8")
        credentials_dict = json.loads(credentials_json)

        # Geçici dosya olarak kaydet (GitHub Actions'ta)
        with open("temp_client_secret.json", "w") as f:
            json.dump(credentials_dict, f)
    except Exception as e:
        logger.error(f"❌ Credentials decode hatası: {e}")
        raise RuntimeError("❌ YouTube kimlik doğrulaması başarısız!")

    # Token'ı yenile
    if creds and creds.expired and creds.refresh_token:
        logger.info("🔄 Token süresi dolmuş, yenileniyor...")
        # refresh_token için credentials gerekli
    ...
    # credentials.json dosyasını sil (güvenlik)
