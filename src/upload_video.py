...
def authenticate_youtube():
    """YouTube API için yetkilendirme yap (BASİT VE GÜVENİLİR)"""
    logger.info("🔑 YouTube API yetkilendirmesi yapılıyor...")
    
    # 1. token.pickle dosyasını kontrol et
    token_path = Config.TEMP_DIR / "token.pickle"
    creds = None
    
    # 2. Token dosyası varsa yükle
    if token_path.exists():
        try:
            with open(token_path, "rb") as token:
                creds = pickle.load(token)
            logger.info("✅ Mevcut token.pickle dosyası yüklendi.")
        except Exception as e:
            logger.warning(f"⚠️ Token dosyası yüklenemedi: {str(e)}")
            creds = None
    
    # 3. Token geçersizse yeniden yetkilendir
    if not creds or not creds.valid:
        logger.error("💡 ÇÖZÜM: YENİ TOKEN OLUŞTURUN VE GITHUB SECRETS'İ GÜNCELLEYİN")
        logger.error("1. PyCharm'da token_olustur.py çalıştırın")
        logger.error("2. Yeni token.pickle dosyasını base64'e çevirin")
        logger.error("3. GitHub Secrets'te YOUTUBE_TOKEN_ENCODED güncelleyin")
        
        # HATA KODU İLE DURDUR (GitHub Actions'da görülebilsin)
        raise RuntimeError("TOKEN_SÜRESİ_DOLMUŞ - YENİ TOKEN GEREKLİ")
